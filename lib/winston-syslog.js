  ...
  this.connect(function (err) {
    ...
    if (self.isDgram) {
      ...
      if (self.protocol.match(/^udp/)) { 
        ...
        //TODO this really should be "self.path"
        self.socket.send(buffer, 0, buffer.length, self.host, onError);
      }
    }
    else {
...
Syslog.prototype.connect = function (callback) {
  ...
  this.socket.on(readyEvent, function () {
    ...
    if (self.isDgram) {
      ...
      self.socket.write(self.queue.join(''), 'utf8');
    }
  ...
  if(self.protocol.match(/^udp/)) { 
    ...
    //TODO this really should be "self.path"
    return this.socket.bind(this.host);
  }
  else {
