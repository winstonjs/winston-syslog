...
var Syslog = exports.Syslog = function (options) {
  ...
  var isUnixDgram = /^unix/.test(this.protocol);
  if (isUnixDgram  && !this.path) {
  ...
  if (!isUnixDgram) {
    // on unix dgram host is added automatically
    producerOptions.host = this.localhost;
  }

  this.producer = new glossy.Produce(producerOptions);
};
