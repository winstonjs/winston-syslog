...
var Syslog = exports.Syslog = function (options) {
  ...
  this.path     = options.path     || '/dev/log';
  this.app_id   = options.app_id   || process.title;
...
Syslog.prototype.connect = function (callback) {
  ...
  //
  if (this.socket) {
    if (!this.socket.readyState) return callback(null);
  ...
  //
  if (/^udp/.test(this.protocol)) {
    this.socket = dgram.createSocket(this.protocol)
    return callback();
  }
  else if (/^unix/.test(this.protocol)) {
    ...
    //
    callback('Not yet ready to send message');
    ...
    // was just created.
    //
    this.socket.on('connect', send).on('error', function (ex) {
      //
